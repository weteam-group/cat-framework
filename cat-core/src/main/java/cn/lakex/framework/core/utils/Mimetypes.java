...
public class Mimetypes {
            ...
            if (line.startsWith("#") || line.length() == 0) {
                ...
                if (st.countTokens() > 1) {
                    ...
                    while (st.hasMoreTokens()) {
                        ...
                        extensionToMimetypeMap.put(StringUtilPro.lowerCase(extension), mimetype);
                        if (log.isDebugEnabled()) {
                            log.debug("Setting mime type for extension '" + StringUtilPro.lowerCase(extension) + "' to '" + mimetype + "'");
                        }
                    }
                } else {
                    if (log.isDebugEnabled()) {
    ...
    public String getMimetype(String fileName) {
        ...
        if (lastPeriodIndex > 0 && lastPeriodIndex + 1 < fileName.length()) {
            String ext = StringUtilPro.lowerCase(fileName.substring(lastPeriodIndex + 1));
